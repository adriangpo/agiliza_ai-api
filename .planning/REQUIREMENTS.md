...
- [x] **AUTH-01**: User can register with unique email and password per tenant (RN-001)
- [ ] **AUTH-02**: User can authenticate via OAuth social login — Google; Apple if driver available (RN-001)
- [x] **AUTH-03**: User receives an opaque access token on login (AdonisJS v7 auth v10 access tokens guard, DB-backed); tenant context is loaded from the user record per request — no JWT, no token payload claims
- [x] **AUTH-04**: Access tokens expire; user can obtain a fresh token by re-authenticating (or via refresh token if supported by auth v10 guard)
- [x] **AUTH-05**: Refresh tokens are invalidated on logout; token blocklist stored in Redis
...
| AUTH-01 | Phase 2: Authentication & Identity | Complete |
| AUTH-02 | Phase 2: Authentication & Identity | Pending |
| AUTH-03 | Phase 2: Authentication & Identity | Complete |
| AUTH-04 | Phase 2: Authentication & Identity | Complete |
| AUTH-05 | Phase 2: Authentication & Identity | Complete |
