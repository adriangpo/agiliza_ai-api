...
//
// Transaction note: Accepts ctx.db (tenant-scoped transaction from TenantMiddleware) to avoid
// opening a nested db.transaction() inside the middleware's transaction. This prevents test
// teardown conflicts with Japa's withGlobalTransaction() wrapper.
...
export default class AccountService {
  async deleteAccount(user: User, trx: TransactionClientContract): Promise<void> {
    // AUTH-08: Invalidate all tokens FIRST (before anonymizing email/password)
    // so no token can be used while the deletion is in progress.
    // User.accessTokens.all/delete use the global db connection (no RLS — auth_access_tokens has no RLS).
    const tokens = await User.accessTokens.all(user)
    for (const token of tokens) {
      await User.accessTokens.delete(user, token.identifier)
    }

    // D-17, Step 5: Hard-delete all OAuth identities for this user (within the same transaction)
